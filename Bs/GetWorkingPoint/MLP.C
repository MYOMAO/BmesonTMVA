...
//#include "TMVA_MLP_PbPb_pt_5p0_10p0.class.C"
#include "readxml/weights/TMVAClassification_MLP.class.C"

#define MAX_XB       20000
using namespace std;


void MLP(TString infname, TString ofname, int ptmin, int ptmax)
{
...
//	theInputVars.push_back(a14);
	...
	outf->cd();
	TTree* mvaTree = new TTree(Form("MLP_pt_%d_%d",ptmin,ptmax),"MLP");	

	double MLP[MAX_XB];
	mvaTree->Branch("Bsize",&Bsize,"Bsize/I");
	mvaTree->Branch(Form("MLP_pt_%d_%d",ptmin,ptmax),MLP,Form("MLP_pt_%d_%d[Bsize]/D",ptmin,ptmax));
	...
	for(int i=0;i< t->GetEntries();i++)
	{
		...
		for(int j=0;j<Bsize;j++)
		{
			...
			cout << "Bd0 = " << Bd0[j] << endl;
			cout << "cos(MLPheta) = " << cos(MLPheta[j]) << endl;
	...
	//		inputValues.push_back(cos(MLPheta[j]));
...
int main(int argc, char* argv[])
{
	if(argc==5)
	{
		MLP(argv[1],argv[2],atof(argv[3]), atof(argv[4]));
		return 0;
	}
	else
	{
		std::cout<<"  Error: invalid argument number - MLP()"<<std::endl;
		return 1;
	}
}
